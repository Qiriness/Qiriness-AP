...
3. Apply the eight files in `supabase/migrations/` **in order** with `npm run db:apply:migration supabase/migrations/<file>`: `01_foundation.sql` → `02_shopify.sql` → `03_knowledge.sql` → `04_support.sql` → `05_exemplars.sql` → `06_analytics.sql` → `07_drafting.sql` → `08_testing.sql`. They are a baseline for an empty database, not idempotent patches, and the order is a dependency chain (everything references `shops`; tickets reference customers; exemplars reuse the `french_unaccent` config from 03; analytics and drafting read the support tables).
...
- **The test chat is built and has never been run against live data.** `/agent-setup`
  → **Test the agent**: a message you write goes through the real passes (gate, identity,
  categorisation, investigation with real tools, order, drafting) against an in-memory
  database, so no ticket is written, and the transcript shows every tool call with the exact
  text the model was handed. `08_testing.sql` is applied and verified; what has not happened
  is a run against real Supabase reads and real model calls. `VALIDATION_LOG.md` item 17 has
  the checks, of which the one that matters is **run the same message through the test chat
  and the worker and compare** — the category, tool set and verdict should agree, or the
  transcript cannot be trusted.
