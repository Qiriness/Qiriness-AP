...
comment on column public.knowledge_documents.voice_profile is
  'Structured brand-voice fields for the singleton Brand Voice article (core_topic = ''brand''): { tone: string[], voice: string, dos: string[], donts: string[] }. Empty ({}) on every other article. Always-included drafting-agent context, distinct from content_html (used on this row for freeform general-context guidance) and from ordinary knowledge_documents rows, which are selectively retrieved via knowledge_chunks.';
