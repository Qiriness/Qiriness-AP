-- 37_collection_handles.sql
--
-- The Collection mix card reads the six product RANGES the business manages by
-- (scripts/lib/sales-collections.mjs), not every collection Shopify holds.
-- Ranked by revenue, the 176 collections are mostly machinery — fourteen
-- `Diag - Soins hebdomadaires "rituels" - N` variants and the diagnostic sets
-- the advice layer uses — and they drown the ranges while overlapping each
-- other almost entirely.
--
-- So `insights_collection_sales()` takes `p_handles`: the collections to
-- report, or null for every one whose membership is synced. WHICH HANDLES is a
-- business judgement and stays in JavaScript, as the platform channel lists do.
--
-- Drops 36's seven-argument version and recreates it with the eighth argument,
-- copied byte-for-byte from 06_analytics.sql (37_collection_handles.test.mjs
-- asserts it). Supersedes 36. No table, no data.

drop function if exists public.insights_collection_sales(uuid, timestamp, timestamp, text, text[], text[], integer);
...
-- belong to no collection reported here.
--
-- COLLECTIONS OVERLAP, AND THAT IS NOT A BUG. `advice_collections.product_ids`
-- puts a product in every collection that carries it, so a product's revenue is
-- counted once in EACH of its collections and the shares do not sum to the
-- range. The caller says so on the card; summing these rows would double-count
-- and is never right.
--
-- P_HANDLES NARROWS IT TO THE RANGES the business reads by; null reports every
-- collection whose membership has been synced. Either way the null-id row is
-- everything else that sold, so the card can state what it leaves out rather
-- than implying the collections are the whole catalogue.
...
as $$
  ...
  known as (
    ...
    where c.shop_id = p_shop
      ...
      and (p_handles is null or c.handle = any(p_handles))
  ),
  per_collection as (
    select
...
comment on function public.insights_collection_sales is
  'Per collection for a date range: distinct orders, units and net line revenue, on insights_product_sales line rules, plus one null-id row for paid lines in no reported collection. p_handles limits it to the ranges the business reads by. Collections overlap — a product counts in each of its collections — so the rows never sum to the range.';
