-- 18_product_customer_mix.sql
--
-- Adds the Sales panel's product card: insights_product_customer_mix(), one
-- product's customer split and what else its buyers bought. Copied byte-for-byte
-- from 06_analytics.sql (18_product_customer_mix.test.mjs asserts it).
--
-- THE SIX-ARGUMENT DRAFT IS DROPPED FIRST. It returned every product at once and
-- may already be applied somewhere; `create or replace` cannot add p_product_id
-- or change the returned columns, and leaving both would give PostgREST two
-- overloads to choose between. Both statements are idempotent. No table, no data.

drop function if exists public.insights_product_customer_mix(uuid, timestamp, timestamp, text, text[], text[]);
