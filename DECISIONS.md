...
**The run is latency-bound, not cost-bound, so the page size was the cheap half of the fix.** Measured against the live shop on 12 September: one customer page took 378 ms at `first:10` and 443 ms at `first:50`; one order page 1054 ms at `first:10` and 1359 ms at `first:50`. At 10 — the default the nightly silently took — 58,357 customers and 5,992 orders are roughly 6,400 round trips. The nightly now passes `--page-size=50`. `first:100` was measured working too, on both connections; 50 is the choice because query cost scales with the nested selections and the ceiling is worth keeping headroom against.
