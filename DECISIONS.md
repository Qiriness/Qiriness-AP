...
**One deliberate never-satisfiable entry, down from two.** `checkout_state` had no tool wired — `abandoned-checkout.mjs` existed and was validated but sat outside the investigation registry — so it always resolved `unavailable`, and counting how often it was *needed* was the argument for wiring it or for dropping it. It was wired 2026-09-16; see § "The basket becomes a branch". `other_fact` remains, and always will.
...
**`delivery_delay_state` is a new need, not a fourth `delivery_state` value**, and that was the whole design question. A new value would have done something worse than duplicate: `expediee_sans_scan` branches on `dispatched_no_scan`, so the moment the late case became its own value that rule would have silently stopped matching it — and every stored investigation that recorded `dispatched_no_scan` would have come to mean something narrower than it did when it was written. As a separate need the two are read off the same tool call and compose: `{delivery_state: dispatched_no_scan, delivery_delay_state: overdue}` is two conditions, which beats `expediee_sans_scan`'s one on specificity, so the new rule takes the late slice and the old rule keeps the rest. Nothing existing changed.

**Two numbers, because one would be wrong somewhere.** Measured over the last 1 000 orders: **881 to France, 119 elsewhere** (Belgium 58, Italy 24, the Netherlands 14, then the United States, Switzerland, Monaco, Spain, Hong Kong, Portugal, Luxembourg). A parcel late to Paris is still on time to Milan, so `abroad_delivery_days` joins `france_delivery_days` and the order's `country_code` picks between them. **Monaco is on the abroad number** — La Poste arguably treats it as domestic, and four rows is not enough for this codebase to decide that.

**The reply never quotes either number.** Same split `return_eligibility` already makes: the parameter decides the branch and the customer does not see it. A skeleton quoting `{france_delivery_days}` would be wrong on the Belgian parcel it also fires on, and « le délai habituel est dépassé » says the true thing for both.

**The clock is the customer's message, and this corrected every order state, not just the new one.** `orderStates` compared against `new Date()` — the moment the worker happened to run. What a rule is asking is how things stood WHEN THEY WROTE. The two agree on live mail and part company everywhere else, and the measurement makes the size of it plain: the corpus orders are 16–100 days old (median 55), so against `now` **all 62** dispatched-no-scan tickets read as overdue and the state would have carried no information at all. Against the ticket's latest inbound message the split is **23 overdue / 39 within window**. `ticket.latestInboundAt` now travels on the investigation's ticket and falls back to the wall clock when a message carries no `received_at`, because a missing date must not turn every order state into `unknown`.

**It degrades to today's behaviour rather than to a guess.** No parameter, no country code, no dispatch timestamp, or a parcel not yet gone or already arrived — all `unknown`, and `expediee_sans_scan` keeps the ticket. That included every bundle stored before `delivery.dispatchedAt` existed: `daysSinceDispatch` was frozen against the build-time clock and could not be re-measured, so the raw instant now travels too. It also filled a reader that had been silently empty since it was written — `escalationTriggers` has always read `delivery.dispatchedAt`, and nothing ever set it.

**D-05 does not get the split, and should not.** `D05_expediee_sans_scan` names the situation, and a rule naming one outranks a shared rule before depth is counted. That is the ranking working: D-05 IS « le suivi n'a pas bougé » — the customer has already said it is late — so its own rule already apologises and already routes to a person.
...
### The investigation reads the thread, not the customer's half of it (2026-09-21)

`record.inboundMessages` filters `direction = 'inbound'`, and every pass that read a thread used it. So a follow-up was investigated against the customer's words with our own answers missing: « oui, j'ai vérifié les deux » arrived with no record of what they had been asked to verify. **95 outbound bodies were stored and no pass in this pipeline had ever read one.**

**The reader is new, not widened.** `record.conversation` sits beside `inboundMessages` rather than replacing it, because the two answer different questions and the older one is still right for its own. The categoriser labels what the customer asked; the situation matcher scores the request that opened the thread. Neither wants our replies in its query, and **both were calibrated without them** — the 0.65 band was derived over first inbound messages.

**Everything calibrated on inbound mail still runs on the inbound subset**, filtered in the runner: the trade-sender check, the opening message the matcher reads, the trigger message the case file is keyed to, `summarisePhotoEvidence` and `latestInboundAt`. Only the text handed to the model moved.

**The filter is `!== 'outbound'`, and how it fails is the reason.** A row whose direction did not travel — a projection that lost the column, a caller wiring its own read — stays in the inbound set and the pass degrades to the behaviour it had before it could see our replies. The strict test would empty the set and skip the ticket, which is a customer's mail disappearing to fix a column.

**One message renders bare, with no label and no date.** 133 of 172 tickets are a single inbound message with no reply; rendering them as a labelled transcript would change every one of their prompts to fix the 39 this is for. A test pins the single-message output byte for byte, and that is the safety property for the other 79%.

**The budget is spent newest first**, and charged for the separators and markers rather than only the bodies. An ascending render that ran out of budget would truncate the message the pass was woken up for. It was 4,000 characters for two bodies and is 12,000 for a thread — the longest in the corpus is 9 messages, and an outbound body averages 1,758 characters.

**`quotedText` stays inbound-only.** It exists because a forwarded order confirmation carries the number and the address together; quoted history inside our own replies is our own text coming back, which is the noise the split exists to remove.

### A finished thread is reconstructed in one reading, not replayed (2026-09-21)

The corpus predates all of this: 36 threads carry two or more customer messages and every one of them was read by the pipeline exactly once, at the end, as if the whole conversation had arrived at once. To get a starting state for them there were two options, and replaying the pipeline message by message is the wrong one. It would spend a categorisation, an investigation and a draft **per message** to rebuild a state the finished thread states plainly, and it would spend them against tools whose answers have since moved — an order that was `dispatched_no_scan` in July reads as months late today.

**So `cases:reconstruct` reads the whole thread once**, the way a person picking up a case file would, and reports where it stands.

**It writes nothing, structurally rather than by policy.** The runner is built with a reader and no record module, so there is no write path to forget to guard. The read-only requirement in the brief was mostly free already — every Shopify scope is `read_*`, drafting makes no Graph call, nothing issues refunds — so the only real exposure was writing rows, and the way to not write rows is to hold nothing that can. The one row it does leave is the `llm_usage` line, because the call was billed either way, and the header says READ ONLY rather than "writes nothing" for exactly that reason.

**The model reads prose; code reads the backend; the two are never merged.** It is asked what the conversation *claims* about a refund — `not_mentioned` / `promised` / `stated_done` — and never whether a refund happened. `backendPosition` derives that from `tickets.resolved_context`, read and not re-derived. A customer told « votre remboursement est parti » has been told that, which is a fact about the conversation and evidence of nothing else; the contradiction between the two is the finding worth a person's time, and it only exists while they are kept apart. The order bundle is deliberately absent from the prompt, so the model cannot report a refund as established because Shopify shows one.

**`replacement` is `not_representable`, permanently.** Nothing in the order bundle records "we sent one of these again for free" — a replacement leaves as a fulfilment on another order or on none. The honest value says the question cannot be answered here; the alternative is a field reading `no` that means « nous n'avons pas regardé », which is the `categorisation_confidence` mistake again.

**The situation comes from the shop's own library or is null**, checked in code against the 40 approved `exemplar_key`s. A key the model invents is dropped **and reported**, because that is a fact about the library as much as about the model. Null is a real answer: the library was never written against this corpus.

**Nothing downstream reads the output, and that is the point.** Judging a reconstruction against a reconstruction is circular — the brief's own regression format asks for a `case_state_before_test_message` that only this could produce. It breaks the way `categorisation_review` broke it: the rows are corrected by a person and the corrected set is what a suite rests on.

### The reconstruction's situation choice is not stable, and that is what makes the review load-bearing (2026-09-21)

Run over the same 36 threads twice, with the prompt differing only by an instruction to write in French, the situation tally moved: `(aucune)` **4 → 2**, D-06 5 → 6, D-07 1 → 2, D-36 1 → 0, R-23 0 → 1. **At least three of 36 threads changed situation between runs** on a change that had nothing to do with situations — roughly 8%.

**So the output is a draft reading, never a label.** Correcting it by hand was already the plan, for the circularity reason; this makes it load-bearing rather than tidy. A regression suite built on an uncorrected run would be pinned to whichever way the coin landed that afternoon, and the first unrelated prompt edit would look like a regression.

**It also bounds what the situation half of this is worth.** 34 of 36 threads matched something, which reads well until the instability is priced in — the honest claim is that reconstruction narrows 40 candidates to one plausible answer, not that it identifies the situation.

**Language follows the thread unless it is told not to, and mostly obeys when it is.** The first run summarised several French threads in English; with the instruction, 1 of 36 still came back in English — `d9f5fa74`, the Italian thread. Left as is: it is a review artefact a person is correcting anyway, and chasing the last one is tuning a prompt against a single row.

**Threads with one customer message are skipped by default.** They have no trajectory to reconstruct and their case file already says everything this could; 133 of 172 tickets are exactly that, so the default spends 36 calls instead of 172 to learn the same thing.
...
### A reply is now written knowing what we already said (2026-09-21)

Drafting read the trigger message and the thread's **envelopes** — `'ticket_id,direction,received_at,sent_at'`, no bodies — and that read answered one question, `describesChase`. So a draft could re-ask a question our previous mail had already asked, re-explain what it had already explained, and contradict what it had promised, with nothing in the pipeline able to notice.

**The envelope read was right for its question and became the whole view by default.** It was written to answer "did they write again before we answered", and pulling bodies for that would have shipped every email in the thread to a pass that reads one. It is now `threadForDrafting`, and `describesChase` takes exactly the same rows it always did.

**Our replies go ABOVE the new message**, on the same argument that puts the customer's words before the case file: the model should read what they wrote already knowing what it answers. Placed after, our own reply arrives as a footnote to a question the model has finished interpreting.

**Two blocks with separate budgets**, ours and their earlier messages, so a long complaint cannot crowd out the answer we gave it. Older messages give way **whole** rather than being head-sliced: the commitment — « nous revenons vers vous dès que le transporteur répond » — is usually in a reply's last paragraph, which is exactly what a head-slice drops.

**The related ticket's mail is deliberately not in the transcript.** `thread` merges a sibling ticket's envelopes because the chase spans both, and that is right for the chase. The transcript is a different question — what did we say on *this* thread — and a second conversation spliced into it would read as one.

**This widens the prompt's personal-data surface, deliberately.** An outbound body averages 1,758 characters and carries the customer's name and order numbers. The existing narrowing holds: still no `from_email`, still no `from_name`, because the reply is composed from the case file and an address in the prompt is an address the model can quote back.

**The sign-off is removed from every history message, and that is measured rather than tidy.** Ticket `718086fd` is a Spanish thread whose middle replies are French and end « Bien cordialement, Service Client Qiriness ». Shown those, the model ended its Spanish reply with the French signature and failed the `signature` check, which requires the approved wording *translated* outside French. **Framing did not fix it** — the block already said not to recopy, and the draft recopied anyway. The signature and closing line are approved, stored fields reproduced from the system prompt on every reply, so in the history they carry no information at all and the only thing they can do is be imitated. Same removal `draft-checks` performs before looking for an invented closer.

**The inbound block is « reçus sur ce fil », never « du client ».** It said `du client` for one run, which is wrong for the reason § *Direction: the Inbox is not only inbound* and § *A colleague is not a customer* already give. `c5ec7404` is the case: **four of its five inbound messages are internal staff and Deret coordination**, and the trigger message is a colleague's. Labelling that as the customer writing invites a reply to the wrong person, and the block now says so in as many words.

### What the thread in the prompt is actually worth, measured (2026-09-21)

Re-drafted the 19 threads that carry both a customer reply and one of ours. **All 19 drafts changed.** The result is a real improvement in continuity and a narrow imitation risk, and the honest summary is that the mechanical checks cannot see either.

**The check count is not a usable signal at this size.** `checks_passed` went **14 → 12 of 19**, and `718086fd` passed on one run and failed on the next **with identical code**. A two-point move over 19 samples is inside the model's own variance; anyone quoting it as a regression is quoting noise.

**Register drift is narrow, not general.** Across all 19: the French closer appears in a non-French reply on **1** (`718086fd`), and the title salutation is lost on **1** (`d6d0d1c3`, where our own earlier reply used the first name). Average length 500 → 529 characters.

**What the reading shows.** `8236165a` now says « Je vous confirme que votre commande nous est bien revenue » in the first person our previous reply established, where it used to open as though nothing had been said. `d6d0d1c3` newly acknowledges the mask that is still missing *and* the replacement already sent. `1e4890dd` volunteers the dispatch date and parcel number.

**The one clear content regression is a thread that was already broken.** `c5ec7404` now asks for a screenshot and a photo — a question the case file does not license — on a ticket whose case file was written from a colleague's message and does not know we had already shipped the replacement with a tracking number. The history made an existing data problem visible rather than causing it.

### A re-ask guardrail was built, measured, and removed (2026-09-21)

The obvious check on top of the history block: read our own sent replies through `ASK_TERMS` and flag a draft that asks again. It does not work, and the numbers are why rather than an opinion.

**Measured over the 25 threads carrying both a customer reply and one of ours.** Whole-body matching claimed **2.4 fields per thread** — `product_name` and `reaction_product_name` each fired on 10, always together, because they share the single term « produit », which any reply mentioning a product contains. Narrowed to sentences carrying a request cue it fell to **0.3 per thread**, and about half of what survived was still wrong: « Avez-vous effectué un retour de produit ? » is not a request for the product's name.

**The direction is what breaks it.** `ASK_TERMS` exists to check that a draft DOES ask for something, where a false positive costs a reviewer a minute. Read backwards the failure inverts — a wrong hit suppresses a question the case file licensed, and the customer is never asked for the one fact that would let us help them.

**Not kept as an advisory counter either.** A count nobody can trust is the `categorisation_confidence` mistake: a field that reads like information and is not. The prompt carries the instruction instead, and a real guardrail needs the question recorded when it is **asked** rather than recovered from prose afterwards — which is the case state, not a regex.

### An inbound message is not a customer, and the renderer now says which (2026-09-21)

Three prompts had been told, separately and in prose, that an inbound message might not be from the customer — the drafting history block, the closure check, the reconstruction. A correction repeated in three places is a correction the DATA should have been making.

**`tickets.sender_label` describes the THREAD, from the address that opened it.** A conversation is not made of one sender. Measured: **38 inbound messages from `lap-groupe.com` across 22 tickets**, **14 from Deret across 10**, and **10 from Nocibé** — most of them arriving on threads a customer opened, where the ticket's own label says nothing about them.

**Rendered as « client » or « reçu », they were read as the customer speaking, and three times they were.** A draft asked the customer for a screenshot on `c5ec7404`, whose trigger message is a colleague's. The closure check reported « le client confirme que la commande a été traitée » about `fcf4ca11` — `tlamzouki@lap-groupe.com` writing to another colleague. And `e8903620`, which I had labelled a genuine customer closure, turns out to be `dnouali@lap-groupe.com`: the customer wrote once, at the start, and never closed anything.

**So `senderRole` resolves a role per MESSAGE**, from the directory that already exists, and every renderer uses it: the investigation transcript, the drafting history, the closure prompt, the reconstruction. The vocabulary is the one a reader needs — « client », « collègue (LAP Groupe) », « prestataire logistique », « transporteur », « revendeur », « prestataire » — not the table's internal labels.

**The address travels and is never rendered**, which reverses the narrowing on `messageForDrafting` in letter and keeps it in spirit. That column excluded `from_email` because *an address in a prompt is an address a model can quote back to a customer*; it is now selected, resolved to a role, and dropped. `messageForInvestigation` has always worked this way for the sender-directory lookup.

**An unclassified domain is a customer**, deliberately. A sender nobody has filed is read as demand rather than quietly discounted, and a label with no display name falls back the same way rather than leaking a raw key into a reply.

**The closure check refuses a non-customer in CODE, not in the prompt.** The instruction is the belt and `senderRole !== 'customer'` is the braces — the half that cannot be talked out of it, and the half that costs no model call. It took the code gate from 11 of 16 to **13 of 16** settled before any call, and the eval from 15/16 with one unstable case to **16/16 stable over 5 repeats**.
...
**Measured once it could: 59 of 138 investigations sit on a thread carrying an outbound message newer than the trigger, and every one of them produced a draft.** The queue went 138 → 63 when the check went in. Nearly half of drafting was writing a reply to mail a person had already handled, which is exactly what it looks like from the review side — the last message in the thread is ours, and the draft says the same thing again.

**Strictly newer than the trigger, not "the thread ends with our reply".** The difference is the ordinary case: a customer who writes, is answered, and writes again has an outbound message in the thread and is owed a reply. What disqualifies a draft is a reply that came *after* the message being answered.

**Envelopes only, and no model call.** Direction and timestamp. This is a guardrail, so it is code — a question with one right answer, asked of data already loaded.

**It gets its own helper rather than reusing `timeOf`.** That one returns 0 for a message with no timestamp, which is right where it is used — ordering a thread, where an undated message sorting first is harmless. Here 0 would make an undated *trigger* older than every reply in the thread and suppress a draft on a ticket nobody had answered.

**On this corpus it is mostly history, and not only history.** The mail was imported after a person had worked it, so the human reply predates the agent's first look. On live mail the same shape appears whenever somebody answers in Outlook before a draft is reviewed — which, with drafting operator-triggered and 120 drafts unread, is the normal case rather than the edge one.

**Counted, not filtered in the query.** `skippedBy.already_answered` is how anybody finds out how far behind the queue is running, and a skip that leaves no number behind cannot be argued with later.

**A consequence for the review queue as it stands: 59 of the 123 stored drafts are on already-answered threads.** They were written before this check existed and are still there. Reading them is reading replies to settled mail.

### A case the customer has closed gets a closing reply, not a full one (2026-09-21)

« La commande a effectivement été livrée. Mes inquiétudes n'étaient pas fondées. Je vous remercie d'avoir répondu à mes messages. » was answered with a reply restating the dispatch date and the parcel number, and inviting the customer to get in touch again. Nothing in it was wrong. None of it was wanted.

**5 of the 16 threads where a customer wrote after our reply are closures** — about a third of exactly the population multi-turn work is for.

**The decision is made in two halves, and the order is the design.** `closureAllowed` asks in **code** whether anything is outstanding on our side: a `missing` entry, a handoff, any verdict but `answerable`. Only when the dossier is clear is the message itself read, by one constrained boolean on the cheap tier.

**`d6d0d1c3` is the case that fixes the order.** « J'ai bien réceptionné le colis. Merci encore » closes the delivery question — and the free mask is still missing, so the case file reads `needs_human`. Asked first, a model reading that message alone says the case is closed, and closes a case that owes the customer something. The code gate never lets the question be asked there. Two more of the seven shortest replies in that set, `5836ab80` and `bdc3f08c`, are short, follow our reply, contain no question mark and carry **new information** — so a structural heuristic would have wrongly closed 2 of 7 as well.

**A closure is not a verdict, and that is why `intent` overrides it rather than joining it.** Every other key in `INTENT_RULES` is a verdict, because until now what a reply should do was a property of the case file alone. Here the case file is `answerable` either way; what makes the reply three lines is the customer having said they need nothing more.

**The facts obligation inverts here, uniquely.** Every other intent set obliges the reply to pass established facts on, because a useful fact withheld is one the customer must ask for again (§ *Every verdict gets a reply*). Once they have said they need nothing, the same fact is padding — so `closing` forbids facts outright and says so, rather than leaving the structural rules to pull the other way.

**Nothing is sent silently and no ticket is closed by this.** A closing reply is still a reply, which is what the § above requires; `draftDisposition` already returns `terminal` for an `answerable` with no handoff, so the status behaviour is unchanged.

**A failure is never a closure.** A throw or a rate limit resolves to `closes: false` and the ordinary reply gets written. The other direction — a 429 quietly shortening a reply to a customer still waiting on us — is the one worth engineering against.

**The delay apology stands down on a closing reply, and is reported rather than dropped.** On `1e4890dd` the thread proves a chase, so the check fires; the message being answered thanks us for replying. An apology for our delay, in the reply that closes the case, answers a complaint the customer has just withdrawn. `passed: null` keeps the line and the reason visible.

**Off by one empty env var.** `AGENT_CLOSURE_MODEL=` leaves the reader unbuilt, and an absent reader means no closure is ever detected — the switch is the absence of the reader rather than a flag inside it, so there is no half-wired state.
...
### Advice comes from collections the team switches on (2026-09-16)

Asked for by the owner after ticket `b7b276f6` was answered with a men's cream. The shop already groups products the way advice needs — by concern *and* by category — in Shopify collections; nothing was syncing them. The sync, the curation table, the tool, the situation and the reply shape all shipped the same day; the curation screen has not.

- **175 collections, and most of them must never answer a customer.** Read live: 62 are numbered diagnostic-quiz buckets (`Diag - Soins quotidiens "essentiels" - 1..32`); others are `Black Friday` (92 products), `Singles day` (84), `soldes hiver` (80), `Offre spéciale sitewide` (102), `OrderlyEmails - Recommended Products` (104), `Smart Products Filter Index - Do not delete` (116). **A product sits in 18 to 30 of them** — one cleanser's 21 memberships contain exactly 3 useful ones. So an activation list is not a convenience: uncurated, every product is in `black-friday` and the intersection means nothing.
- **Membership is read from the collection, not the product, and that is forced.** The obvious build is `products { collections(first: 50) }` on the existing product query. Shopify prices a query before running it and refuses over 1000 points, and that query is already at the ceiling — `PRODUCT_MAX_PAGE_SIZE` records 30 passing and 40 refused at 33 points a node. Multiplying that by fifty is not available at any page size worth having. One request per collection is, and only **activated** collections are ever asked for, so the cost is proportional to what somebody decided was useful rather than to the catalogue.
- **A table, not a column on `products`** — which reverses what I recommended when the question was only about membership. Membership alone would fit on the product; the decisions would not. "May support answer from this?" is a judgement about the *collection*, and it needs somewhere to live.
- **Three columns are the team's and the sync must never write them**: `is_active`, `axis`, `note`. `mapCollectionRow` does not return them and the upsert merges duplicates, so a column absent from the payload is left alone — the same mechanism `promotions.offerable_in_replies` and `products.recommended_for_concerns` rely on. Its own test asserts it, because the failure is silent: a nightly run that reset them would un-activate every curated collection and the agent would simply stop recommending, with nothing failing and nobody told.
- **Liveness is filtered at sync and never frozen.** `product_ids` holds only products Shopify reports `ACTIVE` (`Diag - Rides et ridules` holds 19, of which 18 are live), and the read still joins `products.status` — an archived product has no business one query away from a customer-facing recommendation, and storing a status would go stale the moment somebody archived one.
- **Two axes, because the intersection needs both.** « un sérum pour mes rides » is one `category` requirement and one `concern` requirement, and a tool that could not tell them apart could not relax the right one when nothing sits in both.
- **`products_synced_at` is null on a collection nobody activated**, which reads as "we have never asked" rather than "we asked and it was empty".
- **The membership pass updates rather than upserts.** PostgREST's upsert is `INSERT ... ON CONFLICT`, so it validates against every NOT NULL column even when the conflict target exists; a payload of the two membership columns was rejected for a null `handle`. The catalogue pass owns the row, this one refreshes it.

Measured 2026-09-16 with six collections activated: `Sérums Visage` ∩ `Diag - Rides et ridules` = **5 anti-wrinkle serums**, which is the answer Martine's ticket wanted; `Sérums Visage` ∩ `Diag - Taches` = 1; `Soins Contour des Yeux` ∩ `Diag - Cernes et poches` = 5; and the three-way `Sérums` ∩ `Rides` ∩ `Taches` = 0, which is the case the relaxation step exists for.

**The tool, the situation and the reply shape (same day).**

- **The model names the requirements; code owns everything after.** This overrules a recorded rule — `tool-registry.mjs` says the skin concern *"is not something the model is asked for and cannot be something it invents"*, and § "`recommendProducts` has two sources" repeats it. That reasoning holds while the vocabulary is five cue lists this codebase wrote; it does not survive the thirty-odd collections a shop curates across concerns AND product forms, and « je cherche un sérum pour mes taches » is unreachable through cues. **The constraint moves rather than disappearing**: the activated titles are written INTO the tool's schema, `resolveRequirements` matches only against that same list, and a requirement that is not on it is reported as unmatched rather than resolved to whatever shares a word. The model picks WHICH; code still owns WHAT was found. `toolsFor` is synchronous, so the runner primes the list with `registry.ready()` — unprimed, the argument simply has no vocabulary and the concern path answers as before.
- **`relaxed` is its own outcome, not a flag on `by_collection`.** A rule branching on « we found something, but gave a requirement up » writes a different sentence from one branching on a full match, and `when_conditions` cannot read a boolean buried in `data`.
- **The drop is chosen by what it buys, not by position.** Dropping in a fixed order loses requirements it never needed to: « un sérum pour mes rides et mes taches » has nothing in all three, and dropping `rides` first leaves nothing, forcing a second drop and answering with plain serums — both concerns gone. Each candidate is tried and the first that leaves something standing is the one dropped. Ties break concern-before-category (the customer named the *form* they want, and swapping it hands them a different product), then broadest-first.
- **The tags describe a chosen product; they still may not choose one.** § "Recommending is curation" forbids selecting on tags — `peaux sensibles` is on 52 of 90 sellable products. `product-lines.mjs` uses them only to say who an already-chosen product suits, which is the same sentence the merchandising means. A product matching three of the five readable skin types collapses to « convient à tous les types de peaux », because a cleanser here carries nine at once and listing them reads as evasion.
- **One sentence of `short_description`, never a composed one.** A description invented about a cosmetic is the drafting mistake the whole layer exists to prevent; a product without one is named and nothing is added.
- **PR-29 « Avez-vous l'équivalent de ce produit d'une autre marque ? »** — four French variants, translated to en/es/it/de and embedded through the existing pipeline. Measured: Martine's own words match it at **0.755** against a 0.65 bar (they matched nothing at 0.425 before), and it steals nothing — PR-25 0.723, PR-24 0.687, PR-28 0.683 on their own questions, with PR-29 second at 0.46, 0.37 and 0.34. **Its rules refuse to compare formulas**: we do not know the other brand's, and the answer is what the customer is LOOKING FOR, not a claim about the product they named.

**Two sources for the requirements, and neither is enough alone (2026-09-17).**
...
- **Activation is its own endpoint, the axis and note another.** `setRuleApproval`'s rule applied to a different table: switching a collection on is what lets its products reach a customer, where a note is bookkeeping, and a shared handler would mean correcting a typo could switch something back on after somebody had taken it off.
- **An axis is required to go live and cannot be cleared while live.** The pair "active with no axis" is the one state `chooseProducts` cannot act on, so it is refused from both directions rather than left reachable by going round.
- **Two counts on the screen, because they answer different questions.** Shopify's `productsCount` includes products that are not live and is what tells somebody whether a collection is worth switching on at all; the live count is what the agent can put forward. The live one reads `—`, never `0`, until the membership has been fetched — printing 0 for "never asked" would read as "this collection is empty".
- **`recommended_for_concerns` holds two vocabularies.** Skin-concern keys and live collection handles, in one column, because both say "support should put this product forward for X". `tickableKeys` refuses anything outside the two. The concern keys are legacy as of the same day — see below — and survive only so old ticks are not silently dropped.
...
`buildInput` gives the agent the first and latest inbound message. It gave them raw, so the tools read whatever the customer was replying on top of as theirs — see CHANGELOG for the men's cream that produced. The fix is not a new stripper: `scripts/lib/quoted-reply.mjs` already existed and the embedding and categoriser already used it. This path never called it.

- **Split rather than strip, off ONE boundary.** `splitQuotedReply()` returns `{own, quoted, hasQuote}` from the same `findQuoteBoundary` the stripper uses, and its `own` is asserted equal to `stripQuotedReply`'s output. Two functions with two ideas of where a quote begins is the drift worth spending a test to prevent.
- **The quote is kept because it is evidence.** A forwarded Shopify order confirmation is the only place the order number and the address that order is registered to both appear — which is exactly what `confirmation-evidence.mjs` reads, whole, and why it rescues 6 tickets that verification would otherwise refuse. Deleting the block to fix the product matcher would have broken the order resolver's best source.
- **Nothing in the investigation reads `quotedText` yet**, and that is not an oversight. Order resolution is an earlier, separate pass reading its own `firstInboundByTicket()`, so it was never affected by this bug and is not fixed by this change. The field is carried so the split has one definition in the codebase rather than two.
- **A bare forward keeps the original as `own`.** The stripper's existing refusal to return empty, preserved: a blank question is worse for every reader than a noisy one, and reporting the same text as both halves would double it.

### The basket becomes a branch: `lookupAbandonedCheckout` (2026-09-16)

Asked for by the owner, as a branch on P-17 (« le masque offert ne s'ajoute pas à mon panier ») that turns on **whether we have the basket or not**. That branch could not be written: `checkout_state` was the vocabulary's listed-and-unwired need, so `normaliseConditions` would have dropped `{checkout_state: ["retrieved"]}` and saved a rule with no conditions — a silent duplicate of P-17's catch-all. The tool had to be wired first.

- **It is the one tool that leaves our own database.** Every other tool reads Supabase; this one calls the Shopify Admin API live, because a cart is not synced and cannot be — abandoned checkouts are transient and carry a whole basket, so mirroring them would mean storing the shopping of everyone who ever bounced. The client is built on **first use** (`createShopifyClient` is async and every caller builds the stack synchronously) and kept.
- **No credentials means no tool, not a failing tool.** `buildCheckoutLookup` returns null when the shop has neither an admin token nor client credentials; the registry then deletes the handler, `toolsFor` logs `tool_unavailable`, and `checkout_state` resolves `unavailable` — exactly what it resolved before. Same "skip it loudly" rule `verifyPurchase` already had. A half-configured deployment loses a branch rather than answering from a failed call.
- **No arguments, and the email is the reason.** The ticket carries a *hash* of the sender's address; the plaintext lives on the customer row the hash resolves to. An `email` parameter would be the model choosing whose basket to open, on a string it composed — the mistake `lookupPromotion` had to be closed against, and worse here, because the answer is somebody's shopping. The handler resolves the customer itself rather than trusting that `lookupCustomer` ran, and that second lookup writes the `data_access_events` row this access warrants.
- **Three findings, and the middle one carries the weight.** `retrieved` (we hold a basket) · `unavailable` (we looked; the shop has none for this customer) · `unknown` (nothing looked — the subject has no such tool, or the customer was never identified). Only `found` satisfies the need: a miss is a real answer but it is the *absence* of the fact, and counting it as satisfaction is the laundering `promotion_eligibility: undetermined` is refused for. `unavailable` is a fact a reply may state; `unknown` is not.
- **`basket_unseeable` stays on even when we have the basket**, which looks wrong and is not. Shopify keeps one record per checkout session and **mutates it in place** — measured live: the same id went from one item at 219.87 with `QIRINESS10` to two items at 82.25, `createdAt` unchanged. So what comes back is a snapshot dated `updatedAt`, not the cart the customer is looking at. The rendering dates every basket in its first sentence and says to describe it in the past; `draft-checks` keeps flagging « votre panier contient », which remains a claim about the present we cannot make.
- **Neither the recovery link nor the address reaches the model.** `abandonedCheckoutUrl` opens that basket in the customer's own session; it has no business in a drafting prompt or in a stored diagnostic. Asserted in the registry tests over both `promptText` and `data`.
- **The subtotal shown is before discount**, on the measurement `abandoned-checkout.mjs` already recorded: `subtotalPriceSet` comes back net, so printing it against a minimum requirement would tell a customer whose basket qualifies that they are 20 € short.
- **Added to `order` and to `promotions`.** P-17's exemplar is `category: order` and its `source_note` names promotions as a second subject, and `toolsFor` keys off the **ticket's** category rather than the exemplar's — so the tool has to be in both or the branch fires on only half the tickets that need it.
- **`checkout_state` requires `customer_identity`**, because the lookup matches on an address that lives on the customer row. That is a real prerequisite, not a preference about ordering.
- **A 30-day window, and it bounds a page walk.** Shopify's `abandonedCheckouts(query:)` accepts no email filter, so the lookup fetches a date window and matches in memory; a wide window on a busy store is a lot of pages for one support ticket.

**The three P-17 rules that follow** are written as **drafts**, so nothing changes what the desk sends until they are approved: `retrieved` → a person, with the dated basket in the brief; `unavailable` → ask which address the customer was signed in under (the commonest cause of a miss); and the existing condition-less `p17_panier_non_diagnosticable` stays as the everything-else branch. Verified against the live set: each finding selects its own rule, `unknown` and no-findings both fall to the catch-all, and `auditAnswerSet` over the whole `orders` set returns no problems.

**Two things this does not fix.** P-17's other two declared needs, `promotion_validity` and `promotion_eligibility`, are still always `unknown` on an `order` ticket — `TOOLS_BY_SUBJECT.order` has no promotion tools, and giving it them changes what every order ticket can investigate, so it was not done unasked. And nothing here has been run against a real P-17 ticket: the tool is unit-tested and the vocabulary is proven end to end through `selectAnswer`, but no live investigation has called it.
...
### A calendar month is a period, not a preset (2026-09-22)

Asked for by the owner, beside the presets: pick a month and read that month. `?month=YYYY-MM` resolves to the calendar month by day, and **its comparison is the previous calendar month**, not the previous 31 days — "August against July" is the question a month is asked, and the monthly report asks it. A month still in progress is set against the **same elapsed time** of the month before (the like-for-like rule every preset follows), capped at that month's end so the 31st never compares against a day February lacks.

**The four ways to pick a period are one choice.** A preset, a month and a custom from/to each clear the other two in the URL, so the address never holds two answers; precedence is from/to, then month, then preset, for a hand-typed URL that holds several. The picker offers the first order's month to now, derived from `first_order_at`, never written down.

### Overview, Marketing & funnel, and the monthly report measure nothing new (2026-09-22)

Built from the owner's example report, which ran on mock data. **Every figure on the three is either one another panel already prints or blocked with its reason** — the example's layout was kept, its numbers were not.

- **AOV is net revenue over paid orders** (uncancelled), the Sales definition of revenue over the count `insights_sales_overview` returns. The trend's AOV divides by every order in the bucket, cancelled included — the series function counts them — so it can sit a hair under the tile in a month with cancellations.
- **The bridge reconciles to the cent:** gross = charged + `total_discounts`; refunds = charged − kept on uncancelled orders; net = the revenue every card prints. **"Discounts & gifts", not "discounts"**: this shop's promotions mostly *give a product away* (a 100% line discount) or waive shipping, so `total_discounts` is list value given, not price knocked off. August 2026: €1,191.19 across 71 of 152 orders.
...
The CEOs will be emailed it at the start of each month, and a mail client runs no script. So **every figure is rendered on the server, for both comparisons**; the inline script only adds the tabs and the MoM / YoY switch, and without it every section shows in turn with the month-on-month deltas. Nothing loads from outside the file. `renderSalesReport` (`scripts/lib/sales-report.mjs`) takes data and returns a string, so the download route and a future mail job produce the same file.

- **YoY is the same month a year earlier**, like for like while in progress (`yearEarlier`). A comparison whose window starts before the order history is omitted — "no comparison" — never a quiet month.
- **The newsletter card compares its net in people, not percent.** Net is often negative here (August 2026: +77, −129), and −52 against July's −129 printed as "↑ 59.7%" in the first render — true, and unreadable. Gains and losses still compare as percentages. The dashboard's Marketing card shows the previous net as a count for the same reason. Both are floors from the consent snapshot, and a month before the earliest recorded unsubscribe is blocked, as on Customers.
...
**The route exists and this store answers it.** `shopifyqlQuery` on API 2026-07, with `read_reports`, returned 1,624 sessions over seven days, a 1.60% conversion rate, 3,710 pageviews and a 73.2% bounce rate, monthly sessions back to September 2024, sessions by referrer and UTM, and `total_sales` + `orders` per referring channel. **The scope was already granted** — the live token carries 27 scopes where `shopify.app.toml` declared 26 — so the file was behind the install rather than the install behind the file.

**Six datasets exist.** `sales`, `sessions`, `customers`, `inventory`, `payments`, `discounts` answer; `orders`, `products`, `marketing`, `carts`, `checkouts`, `visits`, `traffic` are refused with *Invalid dataset in FROM clause*. Inside `sessions`: `sessions`, `online_store_visitors`, `conversion_rate`, `pageviews`, `bounce_rate`, `average_session_duration`, **`sessions_with_cart_additions`, `sessions_that_reached_checkout`, `sessions_that_completed_checkout`**, grouped by `referring_channel`, `referrer_source`, `referrer_name`, `utm_*`, `traffic_type`, `landing_page_path` and `human_or_bot_session`.

**THE FUNNEL EXISTS, AND THIS FILE SAID IT DID NOT FOR HALF A DAY.** The first probe asked for `sessions_with_cart_addition` (singular — wrong) alongside `sessions_that_reached_checkout` (right). ShopifyQL's `parseErrors` names **only the invalid columns**, but the query still fails as a whole, and the failure was read as "no cart or checkout dataset exists". That false claim reached `DECISIONS.md`, `CHANGELOG.md`, `README.md`, three cards and a paragraph of the monthly report, all of which told the owner a web pixel would be needed. Measured properly, August 2026 reads **4,164 sessions → 339 with cart additions → 252 reaching checkout → 85 completing it**, and 85/4,164 is exactly the conversion rate Shopify prints. **The rule: a refusal names what is absent and never enumerates what is present, so a multi-column refusal proves nothing about the columns it did not name.** Only `product_views` is genuinely missing, and the probe keeps a query for it so the day it appears, a re-run says so.

**Product ENTRIES are measurable; product VIEWS are not, and the funnel draws the first while saying so.** `landing_page_type` is a typed dimension (`Product`, `Homepage`, `Collection`, `Blog Article`, `Cart`…), so where a session came in needs no URL matching; `landing_page_path` names the page, and its handle matches our own catalogue exactly — all 116 products have one, so the 12 busiest product pages named themselves with no fallback. What it counts is the session's FIRST page: somebody who landed on the homepage and then browsed to a product is not in it. It is drawn in the funnel at the owner's request (2026-09-23) and **kept out of the chain**: it carries its own share of the entry, no "from prior step", and the cart step below it still measures against SESSIONS. Without that the card would have read "cart: 16.9% from prior step" — 339 over 2,006, two populations that do not nest, since the cart step includes sessions that entered anywhere. The row says "entries, not views · outside the chain" and the callout repeats it.

**`FROM products SHOW view_sessions` was re-checked before settling for entries** and is refused with *Invalid dataset in FROM clause — products*: an error naming the DATASET, not a column, so unlike the cart and checkout steps there is nothing hiding behind a mistyped name. `view_sessions` exists on no dataset here, and `product_title` exists only on `sales`, which holds no session metrics. August 2026: **2,006 sessions entered on a product page and converted at 1.2%, against 746 homepage entries at 3.6%** — and 265 blog entries that added nothing to a cart at all.

**The catalogue is discovered by being refused.** ShopifyQL publishes no dataset or column list, and `SHOW *` is a syntax error; what it refuses **by name** is the documentation.

**The selection is introspected, because the published shape was wrong.** The docs' example selects `parseErrors { code message }` and `tableData { rowData }`; on 2026-07 `parseErrors` is a list of **strings** and the rows are `rows`, so the first version of the probe returned *"Selections can't be made on scalars"* for every query and measured nothing. The probe now reads the schema and builds its selection from it, so a rename costs a re-run rather than a debugging session.

**Two definitions to settle before any of it is drawn beside our own figures.** Shopify's `conversion_rate` is storefront sessions over storefront orders, while this app's orders include Amazon and Yves Rocher — so a conversion rate computed here as orders ÷ sessions would count marketplace orders against storefront traffic and read high. And ShopifyQL buckets on its own clock, which is not necessarily the shop-timezone wall-clock every ranged function here uses. Both are reasons the ingestion is a separate piece of work rather than a column added to an existing card.

### Revenue is Shopify's TOTAL sales, and AOV is its NET one (2026-09-23)

The owner checked the dashboard against the admin and asked whether our revenue was gross rather than net. It is neither: `sum(total_price - total_refunded)` is Shopify's **Total sales**, to the cent — August 2026, €10,242.28 on both sides. It is after discounts, but it carries **VAT and shipping**, which for August is €2,052 of the €10,242.
...
- **A fourth figure exists and is left alone, labelled.** Best products, Sales by country, Bought together and the product cards sum line `discounted_total`: VAT-inclusive, after line discounts, no shipping — €9,608.95 for August, between net and total. It ranks products correctly, which is what those cards are for, so the maths stays and the caption says what it is. Converting it would mean apportioning order-level tax across lines, which is an estimate this dashboard would then have to label on every card.
...
**A rate cannot be summed.** Conversion for "the last 30 days" is not the mean of thirty daily rates. Storing daily rows would force it to be reconstructed as a sessions-weighted average — right only while no day is missing, and a second definition to disagree with the one Shopify prints in its own admin. ShopifyQL computes whatever window it is asked for, so the range the reader picked is the range Shopify answers, and the number on the panel is the number on the Analytics page. No table, no backfill, no restatement window for late-attributed sessions.

**The precedent is `lookupAbandonedCheckout`:** live, per question, kept in memory, never written. The same judgement applies — this is aggregate data Shopify already keeps and restates, and mirroring it would buy staleness.

**The costs, and what pays them.** The panel now depends on an external API at render time, so: the whole panel's figures are **one HTTP request** (five ShopifyQL queries aliased into one GraphQL document); the answer is **cached five minutes**, the interval `LiveRefresh` re-renders at, and a blocked answer is never cached; there is an **8-second timeout**; and **every failure is a blocked card with the reason**, never a zero and never a broken page. Measured 2026-09-23: 1.5–1.8 s for a full Overview render including the five queries.

**What it is not, when it is not.** A marketplace platform blocks the whole group with its reason — Shopify Analytics counts storefront traffic, and an Amazon buyer never touched the storefront. An earlier period with no sessions at all gets **no comparison** rather than −100%: a live store with zero sessions in a whole month is unmeasured, not quiet.

**Three definitions that were nearly wrong.**

- **Revenue per session divides STOREFRONT revenue.** The headline revenue includes Amazon and Yves Rocher, whose buyers have no session; dividing that by storefront sessions would flatter every marketplace sale. The panel and the report both read a second orders summary filtered to Shopify for this one figure.
- **Conversion is Shopify's, not ours.** Ours would be orders ÷ sessions, and our orders include marketplaces. Shopify's is storefront on both sides of the fraction, so it is the one shown, labelled storefront.
- **Every sessions query filters to humans, and finding that out took a check against the admin.** `sessions` counts bots; the admin's Analytics → Reports page counts **human sessions**. Read unfiltered, 25 Aug – 23 Sep 2026 gave 6,839 sessions at 1.89% against the admin's 5,076 at 2.46% — a quarter low, and low in the one direction a CEO would notice, since they open that screen. The split is a dimension, `human_or_bot_session`: **1,707 bot sessions converting at 0.23%** sat inside the total. Filtered, the same window reads 5,147 at 2.43%, and the remainder is the day still moving. It composes with GROUP BY and TIMESERIES, so the series and the channel table carry it too; `sales` does not need it, because an order is not made by a bot.

  **How the error was nearly shipped, and what caught it.** The figure was internally consistent, matched between panel and report, and plausible — nothing in the code could have found it. What found it was the owner reading the same period in the Shopify admin and quoting three numbers back. That is the check `VALIDATION_LOG.md` item 15 asks for, and it earned its place: a dozen guessed column names (`human_sessions`, `bot_sessions`, `is_bot`…) had all been refused, and the conclusion drawn from that — that the filtered figure was not exposed — was wrong. A refusal names what does not exist; it does not enumerate what does.
- **Both sides of the channel table group on `referring_channel`.** `sessions` also offers `referrer_source` (`direct` / `search` / `paid` / `email`) and `referrer_name` (`google`, `bing`), but `sales` offers neither. The first build joined `referrer_source` to `referring_channel` and produced a table where almost every row had traffic or money and never both, and no conversion rate at all. Grouping both on `referring_channel` — which `sessions` does support — makes the join exact: August 2026 reads direct 2,582 sessions / €2,031, google 1,958 / €2,973, klaviyo 505 / €402. A channel on one side only keeps its figure and dashes the other, because traffic that produced no order and revenue attributed to a channel the session data never recorded are both findings.

**Weeks are fetched as days.** Nothing documents which day ShopifyQL starts a week on; `date_trunc('week')` here starts on Monday. A chart shifted by one day would be wrong in a way nobody would spot, so week-grain ranges fetch days and fold them into our own buckets. Hour rows come back as UTC instants and are converted to the shop clock — without that, the 24-hour range would draw every bar in the wrong place and still look plausible.

### Product performance is a table, and the collection mix is six ranges (2026-09-23)

Asked for by the owner, from the reference report. `Best products` becomes **Product performance**: the same searchable, country-and-VIP-filtered ranking, drawn as a table — revenue, the change in euros and per cent, orders and units — because six columns say more than a bar, and with **Growth** and **Declines** beside Revenue and Orders.

- **Growth and declines are global only**, and the tabs say so rather than computing something wrong elsewhere. A country list holds each country's top few products, ranked in SQL; the same cut one period earlier is a *different set*, so a product that dropped out of it would read as a total collapse. VIP lists have no previous-period twin at all. In both cases `previousRevenue` is null and the note names the reason.
- **A product that sold nothing last period reads `new`, not +∞.** Every change from zero is infinite, so the percentage column refuses it and the euro column still shows the gain.
- **The pin id stays `best-products` through the rename**, because changing it would forget the pin for anyone who had set one.

**The Collection mix card reports six ranges, not 176 collections.** Ranked by revenue, Shopify's collections are mostly machinery — fourteen `Diag - Soins hebdomadaires "rituels" - N` variants and the diagnostic sets the advice layer uses — and they drown the ranges the catalogue is actually managed by while overlapping each other almost entirely. The six the owner named (Temps Sublime, Source d'Eau, Exception, Active Énergie, Eclat Parfait, Rituel Spa) live in `scripts/lib/sales-collections.mjs` **by handle**, because a title is edited whenever the marketing copy changes and a handle is the URL. `Rituel Spa` is the parent of the four `Rituel Spa <theme>` collections and carries 23 products against their 5-7.

- **Their memberships had never been synced.** `sync-shopify-collections.mjs` fetched products for `is_active` collections only — `is_active` means *switched on for advice*, and all six were false. The sync now fetches both sets and **writes neither flag from the other**: flipping `is_active` to get a sales figure would have changed what the agent recommends.
- **The shares overlap and never sum to 100%**, and the card says so instead of implying a split. A product counts in every range that carries it. August 2026: Temps Sublime 20.7%, Rituel Spa 14.3%, and 37.5% of product revenue in none of the six.
- **The last row is what the ranges miss**, so the card cannot be read as the whole catalogue.
- **Share is of paid product LINE revenue** (€9,608.95 in August), which is neither net nor total sales — the line-revenue caveat that already applies to the product ranking.

### The monthly report compares three ways, and every card follows the switch (2026-09-23)

- **6M ON 6M, not 6M against the six before.** The six-month view replaces every headline with the six months ending in the report month, against **the same six months a year earlier** — August 2026 means March–August 2026 against March–August 2025. It first compared against the immediately preceding half-year, which the owner corrected on 2026-09-23: September–February against March–August is mostly a reading of the seasons, and this shop's are pronounced — November is roughly five times August, so every spring would have looked like a collapse and every autumn like a triumph. Measured: €133,966 against €90,128, +48.3%, where the old basis said −26.3% for the same six months.
- **The headline takes Shopify's `total_sales` where it answered**, not our `revenue`. They are the same measure and agree to the cent in a month with no returns, but ours subtracts a refund from the order it was issued against while Shopify dates it to the refund — August 2025 reads 17,598.78 here and 17,656 there. The report is read beside the admin, so the admin's number wins, and it keeps the card consistent with the bridge below it.
- **The sales bridge is horizontal**: label, bar, figure, one row each. Six vertical columns made the small steps unreadable and wrapped their labels.
- **The newsletter KPI left the overview row.** It is a marketing figure and it has a card of its own in Customers & Products; the row is eight tiles and the newsletter was the one that did not belong beside revenue, orders and conversion.

**The switch moves the whole page, not the chips (2026-09-23).** The first build rendered one set of cards and swapped only the percentage beside it, so a reader on YoY saw the month's new-customer count under a year-on-year chip, and a reader on 6M saw the *month's* product table and platform mix under a half-year's headline. The owner found it. Every section is now rendered **once per comparison** — KPIs, both trend charts, the drivers, the bridge, customers, the newsletter, the product tables, the collection mix, the sales mix and the fulfilment cards — each wrapped in `data-cmp` and shown by CSS. The page is about 8% larger for it; a single HTML file that has to work in a mail client and offline cannot fetch the other two.

- **A card that cannot follow the switch says which window it does cover.** Management signals, the funnel, the channel table and the promotions table are read for the report month alone, and each hint now ends "August 2026 only" or "always August 2026 against July 2026, whichever comparison is selected". Silence there would have read as a figure that moved.
...
**The card leads with open rate, not revenue** (the owner's call, 2026-09-25): the tiles are open rate, click rate, recipients, revenue, and the table is sorted by open rate (ties by recipients). Open rate is unique opens over delivered — Klaviyo's definition — and is inflated by Apple Mail Privacy Protection, which the card's note says. Sends to fewer than 50 recipients are not listed (`MIN_LISTED_RECIPIENTS`, the owner's rule, same day): sorted by rate, a 3-recipient test send opened once topped the table at 100%. They stay in the tiles, and the note counts them.
