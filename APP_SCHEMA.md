...
|   |   |-- api/tickets/[id]/route.ts # GET case file + order facts · PATCH status
|   |   |-- api/tickets/[id]/thread/route.ts # GET the conversation (message bodies)
...
|   |   |                            # Dialog (modal shell: escape, scroll lock, focus, backdrop)
...
|   |   |                            # TicketTable (chevron expands, subject opens the thread) ·
|   |   |                            # TicketDetailPanel (results · order/tracking lines · action;
|   |   |                            # fetches on expand) · TicketThreadDialog (draft + email
|   |   |                            # chain overlay) · DroppedMailTable + DroppedMailDialog
|   |   |                            # (gate decision record; no body exists to show) ·
...
|   |   |-- ticket-detail.ts     # pure, two projections: case-file row -> the expanded row's
|   |   |                        # 3 blocks · resolved_context -> order status/tracking lines
...
|   |                            # getTicketDetail, the latest case file + resolved_context;
|   |                            # getTicketThread, every stored message oldest-first) ·
...
|       |   |                            # buildBodyPatch owns the body cap + expiry clock;
|       |   |                            # purgeExpiredBodies is the retention sweep
|       |   `-- spam-body-backfill.mjs   # re-reads dropped mail from Graph to fill pre-08 rows;
|       |                                # a mailbox-id mismatch stops the run, never "gone"
|       |-- pipeline/
...
|                                # run-spam-body-backfill.mjs (spam:backfill[:dry-run] --limit=N)
...
`-- supabase/migrations/         # BASELINE, run in order against an empty database:
                                 ...
                                 # 08_spam_audit_body.sql  spam_audit.body_text +
                                 #                      body_captured_at/body_expires_at, reversing
                                 #                      02's "never the body". The one migration
                                 #                      that lands on a POPULATED table, so every
                                 #                      statement is IF NOT EXISTS and it rewrites
                                 #                      02's now-wrong table comment
...
- `spam_audit` - one row per spam-gate decision, idempotent on `shop_id` + `graph_message_id`: `outcome` (`kept`/`blocked`), `decided_by` (`blocklist`/`llm`), a one-line `reason`, plus `label`, `model`, `blocklist_rule_id`, and `failed_open` (kept only because the classifier errored). Exists because both passes drop mail before any write, so a blocked email would otherwise leave no trace. Keeps `from_email` + `subject`, and since 08 `body_text` + `body_captured_at` + `body_expires_at` — see the invariant. **The body is null in three distinguishable states** and the dashboard renders each differently: never captured (`body_captured_at` null — the row predates 08, or the backfill could not reach the message), captured and since purged, or a genuinely empty email. The distinction is why the backfill selects on `body_captured_at is null` rather than `body_text is null`: keying on the text would make it silently undo the retention purge on every run. Untriaged replies produce no row.
...
- **The middle section is not tickets.** Dropped mail never reaches the `tickets` table — the gate runs before the ticket write (`agent/src/ingestion/delta-poller.mjs`: "spam is dropped here — never written to the database"), so the only trace is a `spam_audit` row. Since 08 that row carries the body as well as sender, subject and reason, but it is still **not a `ticket_messages` row**, which is why "Add as ticket" is disabled rather than absent: promoting one back means the agent re-fetching it from Graph, and hiding the button would hide that it is recoverable at all.
...
- **A queue row expands into what the agent made of it** (`TicketDetailPanel`, `GET web/app/api/tickets/[id]`). Three blocks and no more: **Results** (a headline read off the verdict, plus the `established` claims), **Order** (the order number plus Shopify's own status and tracking, see below), **Action** (one sentence). The case file's other lists — `unverified`, `do_not_claim`, the tool ledger — are written for the drafting stage; pouring them in here would bury the three lines somebody opened the row to read.
  - **The Order block reads from `tickets.resolved_context`, not from the case file**, and that is why the detail route now selects the column alongside the investigation. Two sources, two projections in `ticket-detail.ts`: order facts exist for tickets the agent never investigated, and a case file exists for tickets with no order at all, so neither read can stand in for the other. `summariseOrderContext` **labels what `buildOrderContext` stored and derives nothing** — re-deriving delivery state in the dashboard would give the app a second opinion about the same parcel, and the two would disagree the first time either changed.
  - **Order status, tracking number and tracking status are text lines that appear only when there is data**, not a fixed row of fields. An unfulfilled order shows two lines, a delivered one shows four. Reserving a slot per field — the shape a table forces — fills the block with dashes, and a dash beside "Tracking number" reads as *there is no tracking* rather than *nothing has been resolved yet*. Order status and tracking status are deliberately **two axes**: Shopify's `order_status` says whether the warehouse dispatched, `delivery.state` says whether the carrier has moved it, and "Fulfilled / Dispatched, no carrier scan yet" is the largest delivery cluster in the corpus. A tracking number links to the carrier URL when the fulfilment carries one, and an order shipped in two parcels shows two.
- **Clicking a subject opens the conversation** (`TicketThreadDialog`, `GET web/app/api/tickets/[id]/thread`) — the draft reply first, the email chain under it. Available from the Queue and Closed tables, i.e. wherever there is a ticket. **The chevron and the subject now do different things**: the chevron expands the row into the agent's reading, the subject takes over the screen with the mail itself. They were one control while there was only one thing to reveal; both are still real buttons, so the keyboard reaches either without the row.
  - **In-app rather than a deep link into Outlook, because the link cannot be built.** Ingestion stores no Graph `webLink` (`sanitizeGraphPayload` keeps ids and addresses and nothing that is a URL into a mailbox), and the mailbox is read with an *application* credential — so a URL assembled from a message id resolves only for someone with that shared mailbox mounted, and is a dead end that looks like a bug for anyone else. The cleaned bodies are already in `ticket_messages`; showing them is cheaper and is the same text every agent pass saw.
  - **Both directions, oldest first.** The Inbox holds the desk's own replies too (123 of 348 messages measured), and a thread showing only the customer's half is exactly what makes flicking to Outlook necessary. Direction is carried by a left rule, not by chat-bubble alignment: these are full-width emails.
  - **The draft section is a placeholder and says so.** `TicketThread.draft` is always `null` today — drafting is Phase 5 and no column or table holds one. The field exists so the section renders where it belongs and the wiring point is one named thing rather than a redesign; the empty state names the reason instead of looking like a failed load.
  - Fetched on open, `no-store`, on its own route rather than folded into `GET /api/tickets/[id]`: the bodies are the heaviest thing on the table and the expanded row never shows them.
- **The Irrelevant section's subject opens the dropped email itself** (`DroppedMailDialog`) — **the message and nothing else**. Verdict, gate, reason and timing are already columns in the row it was opened from, so repeating them would turn the one thing the table cannot show into a footnote on data that is on screen anyway. The dialog answers *should this have become a ticket?*, and past the subject line only the email answers that. The single exception is `failed_open`, kept as a warning banner because it has no column and it changes how the text should be read: a fallback, not a judgement about the email.
  - **A missing body gets one of three sentences, never a blank**: purged (past its retention, decision kept), never captured (offering `npm run spam:backfill`), or genuinely empty. "No body" alone reads as a bug in all three cases and is only actionable in one.
- **Both dialogs share `components/ui/Dialog.tsx`**, extracted when the second consumer arrived rather than up front. It owns only the overlay mechanics that must not drift per copy — Escape to close, body scroll frozen, focus landing inside, and a backdrop click that does not fire when the drag started on the panel — and no layout below the header, so a conversation and a one-screen record share it without either bending to the other's shape.
  ...
  - The row is clickable, but a `tr` cannot be tabbed to or given `aria-expanded`; the chevron is a disclosure `<button>` that carries both, and the action cell and the subject button both stop the click so neither closing a ticket nor opening the thread is also "tell me more". One row open at a time — the panel is several lines tall inside a 26rem scroller.
...
7. Every decision from either gate buffers in a `spam-audit.mjs` collector and flushes once per poll into `spam_audit`, **with the body on a `blocked` outcome** (see the invariant). The flush is best-effort — a failed audit write is logged, never fatal, since a retried poll must not re-drop mail. The **last** thing every poll does is `purgeExpiredBodies`: a PATCH nulling `body_text` past its expiry, never a DELETE, because destroying the row would destroy the trace that a drop happened at all. Also best-effort — a purge failure must not fail a poll that has already ingested mail, and the next poll retries it.
...
- Spam never becomes a **ticket** -- both gates drop mail before the first insert, so `spam_audit` is the only trace of a dropped email and is what makes a wrong drop reviewable at all. **Since 08 that trace includes the body**, reversing the original "never the body" rule: the one question a reviewer has is *should this have become a ticket?*, and a subject line does not answer it -- "Votre commande" is a newsletter or a customer whose parcel is lost, and the corpus contains both. The row was reviewable in principle and not in practice. **The body is kept only on a `blocked` outcome** (a kept email is written to `ticket_messages` in full a moment later; copying it here would duplicate personal data into a second table with a second clock) and **under its own expiry** -- `body_expires_at`, nulled by the worker's per-poll purge, while the decision row is kept indefinitely. Bounded review window, unbounded audit trail; `buildBodyPatch` is the single owner of the cap and the clock so ingestion and the backfill cannot diverge.
