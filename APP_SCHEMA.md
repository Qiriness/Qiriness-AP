...
|   |       |-- ArticleWorkspace.tsx # right pane: source, sync state, category, editor
...
|   |-- sync-shopify-content-catalog.mjs # page+policy names/handles only -> shopify_content_sources
|   |-- sync-shopify-nightly.mjs         # runs the above in order
|   |-- embed-knowledge-chunks.mjs       # embedding reconciler (backfill/retry/model change)
|   |-- apply-supabase-migration.mjs process-shopify-compliance-webhook.mjs
|   `-- lib/
...
|           `-- template-extractors/     # section-type -> semantic units: index (registry), faq,
|                                        # rich-text, media-text, accordion, generic-fallback (low
|                                        # confidence), text-utils, placeholder-strings (denylist)
...
|       |-- lib/                 # logger.mjs (JSON, no PII) · shop.mjs (resolveShopId)
|       |-- llm/openai-client.mjs # Chat Completions + Structured Outputs, injectable fetch/retry
|       |-- ingestion/
...
- `tickets` - one per Graph `conversationId` (unique `shop_id` + `graph_conversation_id`), not per email. Categoriser fills `category`/`secondary_category`, `level` 1-4, `responsible_team` (finance/marketing/sales/logistics/contact), `shopify_order_number`, `customer_id`. `resolved_context` jsonb + `context_resolved_at` is a re-resolvable order/customer bundle for the drafting agent (holds PII, never billing/street address). `priority` 1-5. Retention mirrors `orders` (`resolved_at`/`closed_at`/`archived_at`/`retention_delete_after`); `deleted_at` is the separate compliance soft-delete. Stores only `requester_email_hash` + `requester_name`.
...
- `GET shopify-sources` - the catalog (pages + policies), flagging what is already imported.
- `GET articles` / `POST articles` - list; create empty, or resolve a `sourceId`'s live Shopify content and fill it in.
...
- Nothing auto-writes `knowledge_documents`; the catalog sync only fills `shopify_content_sources`.
- `source_type` -> `manual` **is** the manual-edit lock. No separate flag; resync is then unavailable.
